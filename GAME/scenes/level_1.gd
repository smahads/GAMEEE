...
# CRITICAL: Set this to the exact name of your key node in the scene tree.
@export var required_key_name: String = "Key" 

# Set the path for the next level.
...
func _ready() -> void:
	...
	# We use find_child since the Key node is a direct child of the Level node.
...
func _on_door_area_body_entered(body: Node2D) -> void:
	...
	if body is Player: 
		...
		if not is_instance_valid(key_node):
			...
			# Key collected! Teleport.
			if next_level_path:
				get_tree().change_scene_to_file(next_level_path)
				
		else:
			# Door is locked. We use 'pass' to fulfill Godot's indentation requirement
			pass
